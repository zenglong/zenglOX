...
int ping_get_gateway_mac(UINT8 * gw_mac, TASK * task, UINT32 timeout)
{
	...
	if(syscall_network_send((UINT8 *)&test_packet, sizeof(test_packet)) == -1)
	{
		...
		syscall_cmd_window_write("send arp request to gateway and wait for reply\n");
	...
	while(TRUE)
	{
		...
		if(buf_packet->op != 2)
			continue;
		syscall_cmd_window_write("the gateway ip [");
		for(i=0;i < 4;i++)
		{
...
ping_send:
	...
	{
		syscall_cmd_window_write("send icmp request time out");
		return -1;
	}
	else
	{
		syscall_cmd_window_write("sending ... ");
	}
	...
	while(TRUE)
	{
		...
		if(ret != 1)
		{
			...
			if((curtick - origtick) > timeout)
			{
				syscall_cmd_window_write("reply time out curtick:");
				syscall_cmd_window_write_dec(curtick);
				syscall_cmd_window_write(" origtick:");
				syscall_cmd_window_write_dec(origtick);
				syscall_cmd_window_write("\n");
		...
		{
			syscall_cmd_window_write("Destination Unreachable\n");
			if(infinite == TRUE)
				goto ping_send;
			ping_num--;
			if(ping_num == 0)
		...
		syscall_cmd_window_write("Reply from [");
		for(i=0;i < 4;i++)
		{
			syscall_cmd_window_write_dec(((UINT8 *)&buf_icmp_packet->ip.ip_src)[i]);
			if(i < 3)
				syscall_cmd_window_write(".");
		}
		syscall_cmd_window_write("] bytes=");
		UINT32 bytes;
		...
		syscall_cmd_window_write_dec(bytes);
		syscall_cmd_window_write(" ticks=");
		curtick = syscall_timer_get_tick();
		syscall_cmd_window_write_dec(curtick - origtick);
		syscall_cmd_window_write(" TTL=");
		syscall_cmd_window_write_dec(buf_icmp_packet->ip.ip_ttl);
		syscall_cmd_window_write("\n");
		if(infinite == TRUE)
			goto ping_send;
		ping_num--;
...
int main(TASK * task, int argc, char * argv[])
{
	if(argc == 4)
	{
		...
		{
			syscall_cmd_window_write("you have no route gateway ip, you must use 'dhcp' or 'ipconf' command"
		...
		syscall_cmd_window_write("usage: ping num timeout_sec destip");
	return 0;
}
