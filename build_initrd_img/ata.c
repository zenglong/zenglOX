...
int main(VOID * task, int argc, char * argv[])
{
	...
	if(argc == 2)
	{
		if(strcmp(argv[1],"-s")==0)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				{
				case 0:
					syscall_cmd_window_write("ata0-master(ide_index:0):");
					break;
				case 1:
					syscall_cmd_window_write("ata0-slave:(ide_index:1)");
					break;
				case 2:
					syscall_cmd_window_write("ata1-master:(ide_index:2)");
					break;
				case 3:
					syscall_cmd_window_write("ata1-slave:(ide_index:3)");
					break;
				}
				if(ide_devices[i].Reserved == 1)
				{
					if(ide_devices[i].Type == IDE_ATA)
						syscall_cmd_window_write(" ATA Drive ");
					else
						syscall_cmd_window_write(" ATAPI Drive ");
					//if(ide_devices[i].Type == IDE_ATA && !(ide_devices[i].CommandSets & (1 << 26)))
					if(ide_devices[i].Type == IDE_ATA && (ide_devices[i].Size != 0))
					{
						syscall_cmd_window_write_dec(ide_devices[i].Size * ATA_SECTOR_SIZE / 1024 / 1024);
						syscall_cmd_window_write("MB and Last LBA is ");
						syscall_cmd_window_write_dec(ide_devices[i].Size);
					...
					else if(ide_devices[i].Type == IDE_ATAPI)
					{
						...
						if(ata_ret == -1)
							syscall_cmd_window_write("read capacity failed!");
						else
						{
							syscall_cmd_window_write("last LBA is ");
							syscall_cmd_window_write_dec(readCapacity.lastLBA);
							syscall_cmd_window_write(" block Size is ");
							syscall_cmd_window_write_dec(readCapacity.blockSize);
						}
						syscall_cmd_window_write(" - ");
					}
					else
						syscall_cmd_window_write(" - ");
					syscall_cmd_window_write((CHAR *)ide_devices[i].Model);
					syscall_cmd_window_put('\n');
				}
				else
					syscall_cmd_window_write(" no Drive here \n");
			}
			return 0;
		}
		else if(strcmp(argv[1],"-h")==0)
		{
			syscall_cmd_window_write("usage: ata [-h][-s][-r ide_index lba show_num]\n"
	...
	else if(argc == 5 && strcmp(argv[1],"-r")==0)
	{
		...
		{
			syscall_cmd_window_write_hex((UINT32)(buffer[i]));
			syscall_cmd_window_put(' ');
		}
		syscall_cmd_window_put('\n');
		syscall_ufree(buffer);
	...
	syscall_cmd_window_write("usage: ata [-h][-s][-r ide_index lba show_num][-w ide_index lba data...]");
	return 0;
}
